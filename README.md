...
**v1.5.0** — Local web app for your options portfolio: live market data, Greeks, Monte Carlo simulation, risk decomposition (component VaR, dollar-greeks, pin-risk calendar, portfolio market-shock), factor analytics (implied-vs-realized vol, sector exposure, SPY benchmark), an interactive **Greeks Lab** (per-leg or whole-underlying — 3D surfaces, Θ–Γ gamma-rent, Taylor P&L, cross-Greek), drawdown & trade-cohort analytics, premium-adjusted cost basis, orders & rules, tax lots, a trade journal, and broker position sync.
...
Release: **v1.5.0** — see [CHANGELOG.md](CHANGELOG.md).
...
|--------|-------|-------------|
| `GET` | `/api/version` | `{"name":"options-dashboard","version":"1.5.0"}` |
