...
- **Tax-lot analysis inflated every stock trade 100x** — `compute_tax_lots` applied the options contract multiplier (×100) to any lot whose `opt_type` was not exactly `"equity"`. Share round-trips carry `opt_type = "Stock"`, so all stock proceeds/cost/gain were multiplied by 100 (net realized showed −$108,146 instead of −$15,974; the stock portion alone went from −$931 to −$93,103). The multiplier is now applied only to actual Call/Put lots; shares use ×1
