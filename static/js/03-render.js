...
function compressStrategy(strategy) {
  ...
  const shown = entries.slice(0, 3).map(([k, v]) => v > 1 ? `${k} ×${v}` : k).join(" + ");
...
function renderPortfolio(portfolio, hasMarket) {
  ...
  if (state.greeks && state.greeks.portfolio) {
    ...
    if (state.greeks.risk) {
      html += `<div class="stat" style="border-left:3px solid var(--err-tx)"><div class="stat-label">Max Loss (est)</div><div class="stat-val" style="font-size:16px;color:var(--err-tx)">$${state.greeks.risk.totalMaxLoss.toLocaleString()}</div></div>`;
    }
  ...
  document.querySelectorAll(".tk-block.clickable").forEach(el => {
    el.addEventListener("click", () => jumpToTickerFromPositions(el.dataset.ticker));
  });
  ...
  document.querySelectorAll(".btn-roll").forEach(el => {
    el.addEventListener("click", (e) => {
      e.stopPropagation();
      const pos = rollPosMap[el.dataset.rollKey];
...
function renderTickerHeader(tg) {
  ...
  if (md?.exDivDate) {
    ...
    const daysToExDiv = Math.ceil((exDiv - new Date()) / 86400000);
...
function renderStrike(sg, posType, tg) {
  ...
  if (isEquity) {
    ...
    if (sg.contracts < 0 && (sg.lots?.length || sg.avgCost > 0)) {
      ...
      if (mk?.mid > 0 && avgFill > 0) {
        const pctMax = Math.min(100, Math.max(0, ((avgFill - mk.mid) / avgFill) * 100));
        ...
        premBadge = `<span style="font-size:10px;font-family:var(--mono);color:${pctColor}">${pctMax.toFixed(0)}% max · mid $${mk.mid.toFixed(2)}${dte50}</span>`;
      } else {
