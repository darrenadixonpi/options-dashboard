...
function parseIBKRHistoryRow(r, headers) {
  ...
  const qtySigned = parseInt(parseFloat((r[qtyIdx] || "0").replace(/,/g, ""))) || 0;
...
function parseSchwabPositions(text) {
  ...
  for (let i = hdrIdx + 1; i < lines.length; i++) {
    ...
    const qty = parseInt(parseFloat((r[qtyIdx] || "0").replace(/,/g, ""))) || 0;
    if (!qty) continue;
...
function parseFidelityRaw(text) {
  ...
  for (let i=1;i<lines.length;i++) {
    ...
    const qty=parseInt(parseFloat(r[4]||"0"))||0; if(!qty) continue;
...
function parseFidelityHistory(text) {
  ...
  for(const line of lines){const r=parseCSVLine(line);if(r.length<8)continue;
    ...
    fills.push({date:new Date(parseInt(dp[2]),parseInt(dp[0])-1,parseInt(dp[1])),ticker:p.ticker,expiry:p.expiry,strike:p.strike,optType:p.optType,quantity:Math.abs(parseInt(parseFloat(r[6])))||0,price:parseFloat(r[5])||0});
  }
  return fills;
}
...
function buildHistoryNetQty(histText) {
  ...
  if (fmt === "ibkr") {
    ...
    for (let i = hdrIdx + 1; i < lines.length; i++) {
      ...
      const qty = Math.abs(parseInt(parseFloat((r[qtyIdx] || "0").replace(/,/g, "")))) || 0;
      const code = codeIdx >= 0 ? (r[codeIdx] || "").toUpperCase() : "";
