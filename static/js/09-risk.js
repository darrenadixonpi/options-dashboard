...
function setRiskDaysForward(days, reload = false) {
  const slider = document.getElementById("risk-days-slider");
  ...
  document.querySelectorAll(".risk-expiry-chip").forEach(btn => {
    btn.classList.toggle("active", parseInt(btn.dataset.days, 10) === d);
  });
...
function renderRiskExpiryCheckpoints() {
  ...
  el.querySelectorAll(".risk-expiry-chip").forEach(btn => {
    btn.addEventListener("click", () => setRiskDaysForward(btn.dataset.days, true));
  });
  const cur = parseInt(document.getElementById("risk-days-slider")?.value, 10) || 0;
...
function enableRiskTab() {
  ...
  const wiT = document.getElementById("wi-ticker");
...
async function loadRiskMatrix() {
  if (!state.marketData || !state.positions.length) return;
  const btn = document.getElementById("btn-risk-matrix");
  if (btn) { btn.disabled = true; btn.textContent = "Loading..."; }
  const daysFwd = parseInt(document.getElementById("risk-days-slider")?.value) || 0;
  ...
  try {
    ...
    const firstTicker = document.getElementById("vol-surface-ticker")?.value;
...
document.getElementById("risk-days-slider")?.addEventListener("change", () => {
  setRiskDaysForward(document.getElementById("risk-days-slider")?.value, true);
});
document.getElementById("risk-days-slider")?.addEventListener("input", (e) => {
  setRiskDaysForward(e.target.value, false);
});

document.getElementById("vol-surface-ticker")?.addEventListener("change", (e) => { loadVolSurface(e.target.value); });
...
function _renderVolSurfaceChart() {
  ...
  const volType = activeBtn?.dataset.voltype || "puts";
