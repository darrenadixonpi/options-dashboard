...
export function reconstructSharePositions(positions, histText) {
  ...
  for (const line of lines) {
    ...
    const qty = isSchwabRow
      ? (Math.abs(Math.trunc(parseFloat(c4.replace(/,/g, "")))) || 0)
      : (Math.abs(Math.trunc(parseFloat(r[6] || "0"))) || 0);
...
export function detectStrategies(positions) {
  ...
  for (const [ticker, allLegs] of Object.entries(tickerGroups)) {
    const equityLegs = (allLegs as any[]).filter(l => l.posType === "equity");
    const optionLegs = (allLegs as any[]).filter(l => l.posType !== "equity");
    ...
    const tickers=tkOrd.map(ticker=>{
      ...
      const strikes=tPos.map(pos=>{
        ...
        const portfolioLots=!lots.length && pos.avgCost>0 ? [{
          ...
          dte:Math.max(0,Math.round(((pos.expiry as any)-(new Date() as any))/864e5)),
