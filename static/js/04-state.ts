...
export function renderTickerSearchResults(q) {
  ...
  box.querySelectorAll("[data-jump-ticker]").forEach(btn => {
    btn.addEventListener("click", () => {
      document.getElementById("ticker-search-overlay").hidden = true;
      jumpToTickerContextual((btn as HTMLElement).dataset.jumpTicker);
    });
  });
}
...
export function exportAllSimPathCharts() {
  return (async () => {
    ...
    const canvasIds = Array.from(document.querySelectorAll("#ticker-path-charts canvas[id^='path-']")).map(c => c.id);
...
export function exportHtmlTablePng(tableEl, baseName) {
  ...
  const colCount = Math.max(...Array.from(rows, (r: any) => r.cells.length));
...
export function initChartExportHandlers() {
  ...
  document.addEventListener("click", (e) => {
    const btn = (e.target as HTMLElement).closest("[data-export-canvas], [data-export-el], [data-export-all-paths]");
    if (!btn) return;
    e.preventDefault();
    e.stopPropagation();
    ...
    const canvasId = (btn as HTMLElement).dataset.exportCanvas;
    const elId = (btn as HTMLElement).dataset.exportEl;
    const name = (btn as HTMLElement).dataset.exportName || canvasId || elId;
...
export function getJournalStatsForView() {
  ...
  if (!hasFilter && state.tradeHistory?.stats) {
    return { ...(state.tradeHistory as any).stats, filtered: false };
  }
  const computed: any = computeJournalStats(all, matched);
  if (computed) {
    const activeGroups = new Set(matched.map(journalGroupId));
...
export function renderExpiringRail() {
  ...
  const legs = state.positions.filter(p => p.posType !== "equity" && p.expiry).map(p => {
    ...
    const dte = Math.ceil(((exp as any) - (today as any)) / 86400000);
  ...
  el.querySelectorAll("[data-leg-jump]").forEach(btn => {
    btn.addEventListener("click", () => jumpToLeg((btn as HTMLElement).dataset.legJump));
  });
}
...
export function renderCatalystsRail() {
  ...
  el.querySelectorAll("[data-jump-tkr]").forEach(btn => {
    btn.addEventListener("click", () => jumpToLeg(null, (btn as HTMLElement).dataset.jumpTkr));
  });
}
  ...
  if (!alerts.length) {
    ...
    el.querySelectorAll(".alert-dismiss").forEach(btn => {
      btn.addEventListener("click", (e) => {
        e.stopPropagation();
        dismissAlert((btn as HTMLElement).dataset.alertKey);
      });
    });
  }
...
export function renderAlertHistoryRail(el) {
  ...
  el.innerHTML = events.slice(0, 8).map((ev: any) => {
...
export function saveAlertThresholds() {
  state.alertThresholds = {
    ...
    ivRank: parseFloat((document.getElementById("alert-th-ivr") as HTMLInputElement | null)?.value) || DEFAULT_ALERT_THRESHOLDS.ivRank,
    exDivDays: parseInt((document.getElementById("alert-th-exdiv") as HTMLInputElement | null)?.value, 10) || DEFAULT_ALERT_THRESHOLDS.exDivDays,
    portfolioPProfit: parseFloat((document.getElementById("alert-th-port-pp") as HTMLInputElement | null)?.value) || DEFAULT_ALERT_THRESHOLDS.portfolioPProfit,
    tickerPProfit: parseFloat((document.getElementById("alert-th-tkr-pp") as HTMLInputElement | null)?.value) || DEFAULT_ALERT_THRESHOLDS.tickerPProfit,
    marksStaleMin: parseFloat((document.getElementById("alert-th-marks") as HTMLInputElement | null)?.value) || DEFAULT_ALERT_THRESHOLDS.marksStaleMin,
    bookDeltaAbs: parseFloat((document.getElementById("alert-th-book-delta") as HTMLInputElement | null)?.value) || DEFAULT_ALERT_THRESHOLDS.bookDeltaAbs,
    bookVegaAbs: parseFloat((document.getElementById("alert-th-book-vega") as HTMLInputElement | null)?.value) || DEFAULT_ALERT_THRESHOLDS.bookVegaAbs,
    tickerDeltaAbs: parseFloat((document.getElementById("alert-th-tkr-delta") as HTMLInputElement | null)?.value) || DEFAULT_ALERT_THRESHOLDS.tickerDeltaAbs,
    bookThetaBelow: parseFloat((document.getElementById("alert-th-book-theta") as HTMLInputElement | null)?.value) || DEFAULT_ALERT_THRESHOLDS.bookThetaBelow,
  };
  state.alertNotifyOnFetch = !!(document.getElementById("alert-notify-fetch") as HTMLInputElement | null)?.checked;
  saveSession();
  refreshDeskAlerts();
...
export function setupKeyboardShortcuts() {
  ...
  document.getElementById("ticker-search-input")?.addEventListener("input", (e) => renderTickerSearchResults((e.target as HTMLInputElement).value));
  ...
  document.getElementById("ticker-search-overlay")?.addEventListener("click", (e) => {
    if ((e.target as HTMLElement).id === "ticker-search-overlay") (e.target as HTMLElement).hidden = true;
  });
  ...
  document.getElementById("hist-filter-ticker")?.addEventListener("input", (e) => {
    state.journalFilter = (e.target as HTMLInputElement).value;
  ...
  document.getElementById("hist-filter-strategy")?.addEventListener("change", (e) => {
    state.journalStrategyFilter = (e.target as HTMLInputElement).value;
    if (state.tradeHistory) renderTradeHistory(state.tradeHistory);
  });
  ...
  document.getElementById("hist-show-assignment-legs")?.addEventListener("change", (e) => {
    state.journalShowAssignmentLegs = !!(e.target as HTMLInputElement).checked;
    saveSession();
    if (state.tradeHistory) renderTradeHistory(state.tradeHistory);
  });
